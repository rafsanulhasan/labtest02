...
{
	public class FetchDataServiceEffect
		: Effect<FetchDataServiceAction>, IDisposable
	{
		private readonly HttpClient _http;
		private readonly CompositeDisposable _disposables;
		private readonly FetchDataServiceState _initialState;
		private bool _disposedValue;

		public FetchDataServiceEffect(
			IHttpClientFactory factory,
			IState<FetchDataServiceState> state
		)
		{
		...
		protected override async Task HandleAsync(
			FetchDataServiceAction action,
			IDispatcher dispatcher
		)
		{
			var resultAction = new FetchDataServiceResultAction(_initialState.WeatherForecasts, _initialState.IsLoading);
			_ = Observable
